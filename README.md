...
- A live dependency-failure test (stopping neo4j mid-session against the real running stack, not a mocked failure) found that whether a dead Neo4j surfaces as a 200 (one agent still degrades to a real, data-backed answer) or a 503 (both_failed) depends entirely on whether the specific question's RAG search happens to find any patients before the clinical agent's own RAG-first flow ever reaches Neo4j - not obvious from reading the code, only from actually triggering it. Documented inline in `app/api.py` near the `both_failed` check.
...
- Revisit two accepted `docker scout cves` findings on a future rebuild: `perl` 5.40.1-6 (2 CRITICAL + 2 HIGH, inherited from `python:3.11-slim`'s own Debian layer, no upstream fix yet), and pip's own internally-vendored `setuptools`==70.3.0/`msgpack`==1.1.2 (2 HIGH, `pip/_vendor/vendor.txt`). The second pair only exists because fixing an *earlier* HIGH pair (`wheel`/`jaraco-context`, vendored inside the base image's old `setuptools`) required upgrading `pip` itself - the upgrade step (all three Dockerfiles) stays, since it's a real fix for what it targets, but the newest `pip` release available right now ships this different vendored pair with no fix of its own yet either.
